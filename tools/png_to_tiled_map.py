...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Emit C source containing a flat tile index array and metadata constants "
            "for direct inclusion in runtime map rendering code"
        ),
    )
    parser.add_argument(
        "--c-symbol",
        help=(
            "Base C symbol name for emitted layout arrays/constants. "
            "Defaults to a sanitized form of --name/input stem."
        ),
    )
    return parser.parse_args()


def _sanitize_c_identifier(raw: str) -> str:
...
def _format_c_u8_array(rows: Sequence[Sequence[int]]) -> str:
    lines: List[str] = []
    for row in rows:
...
def _write_c_layout(
    ...
    *,
    ...
    array_type = "unsigned char" if len(result.unique_tiles) <= 256 else "int"
    content = (
        "/* Auto-generated by tools/png_to_tiled_map.py */\n"
        f"/* Source image: {source_image} */\n"
        f"/* Tilesheet: {tilesheet_name} */\n\n"
        f"const {array_type} {symbol}_map[] = {{\n"
        f"{_format_c_u8_array(result.tilemap_rows)}\n"
        "};\n\n"
        f"const int {symbol}_map_width = {result.map_width};\n"
        f"const int {symbol}_map_height = {result.map_height};\n"
        f"const int {symbol}_tile_width = {result.tile_width};\n"
        f"const int {symbol}_tile_height = {result.tile_height};\n"
        f"const int {symbol}_unique_tile_count = {len(result.unique_tiles)};\n"
    )
...
def main() -> int:
    ...
    c_symbol_base = _sanitize_c_identifier(args.c_symbol if args.c_symbol else base_name)
    ...
    try:
        ...
        if args.split_screen_width is not None:
            ...
            for segment in segments:
                ...
                if args.emit_c_layout:
                    c_path = output_dir / f"{part_name}_map.c"
                    _write_c_layout(
                        c_path,
                        result=segment.result,
                        symbol=f"{c_symbol_base}_part{segment.index:02d}",
                        source_image=input_path,
                        tilesheet_name=tilesheet_path.name,
                    )
            ...
            if args.emit_c_layout:
                c_path = output_dir / f"{base_name}_map.c"
                _write_c_layout(
                    c_path,
                    result=result,
                    symbol=c_symbol_base,
                    source_image=input_path,
                    tilesheet_name=tilesheet_path.name,
                )
    except TileMapError as exc:
    ...
    if args.split_screen_width is not None:
        ...
        for segment in segments:
            ...
            if args.emit_c_layout:
                print(f"map c: {output_dir / f'{base_name}_part{segment.index:02d}_map.c'}")
        ...
        if args.emit_c_layout:
            print(f"map c: {output_dir / f'{base_name}_map.c'}")
    return 0
